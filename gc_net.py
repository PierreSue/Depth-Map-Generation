...
class GC_NET(nn.Module):
    ...
    def forward(self, imgLeft,imgRight):
        batch = imgLeft.shape[0]
        imgl0=F.relu(self.bn0(self.conv0(imgLeft))) # b,c,w,h
        imgr0=F.relu(self.bn0(self.conv0(imgRight)))

        imgl_block=self.res_block(imgl0) # b,32, w/2, h/2
        imgr_block=self.res_block(imgr0)

        imgl1=self.conv1(imgl_block) # b, 32, w/2, h/2
        imgr1=self.conv1(imgr_block)
        # cost volume
        cost_volum = self.cost_volume(imgl1,imgr1) # b, maxdisp ,32+32, w/2, h/2 + maxdisp
        conv3d_out=F.relu(self.bn3d_1(self.conv3d_1(cost_volum))) # layer19 b, 32, maxdisp, w/2, h/2
        conv3d_out=F.relu(self.bn3d_2(self.conv3d_2(conv3d_out))) # layer20
        #conv3d block
        conv3d_block_1=self.block_3d_1(cost_volum) # layer2
        conv3d_21=F.relu(self.bn3d_3(self.conv3d_3(cost_volum))) #layer21
        conv3d_block_2=self.block_3d_2(conv3d_21) # layer24
        conv3d_24=F.relu(self.bn3d_4(self.conv3d_4(conv3d_21))) #layer26
        conv3d_block_3=self.block_3d_3(conv3d_24) # layer2
        conv3d_27=F.relu(self.bn3d_5(self.conv3d_5(conv3d_24))) 
        conv3d_block_4=self.block_3d_4(conv3d_27) # layer33
        ...
        out=deconv3d.view(batch, self.maxdisp*2, self.height, self.width)
    ...
    def cost_volume(self,imgl,imgr):
        ...
        for d in range(self.maxdisp):  # maxdisp+1 ?
            ...
            xx_temp = torch.cat((xleft, xright), 1) # b, 32+32, w/2, h/2 + maxdisp
        ...
        return xx0 # b, maxdisp ,32+32, w/2, h/2 + maxdisp
